...
class Sensor:
    def __init__(self, words=None):
        ...
        if type(words) == str:
            ...
            self.badword(custom_words=words)

    ## PUBLIC ##
    def beep(self, text, censor_char="*"):
    ...
    def badword_from_file(self, filename, **kwargs):
        words = baca_badword(filename)
    ...
    def add_censor_words(self, custom_words):
        ...
        for w in custom_words:
            self.CENSOR_WORDSET.append(Variasi(w, char_map=self.CHARS_MAPPING))

    def contains_profanity(self, text):
        """Return True if  the input text has any swear words."""
        return text != self.beep(text)
    ...
    def _populate_words_to_wordset(self, words, *, whitelist_words=None):
        ...
        for word in set(words):
            ...
            all_censor_words.append(Variasi(word, char_map=self.CHARS_MAPPING))
    ...
    def _hide_swear_words(self, text, censor_char):
        ...
        for index, char in iter(enumerate(text)):
            ...
            # Iterate the next words combined with the current one to check if it forms a swear word
            next_words_indices = self._update_next_words_indices(
                text, next_words_indices, index
            )
            contains_swear_word, end_index = cek_kata_sambung(
                cur_word, next_words_indices, self.CENSOR_WORDSET
            )
            if contains_swear_word:
                cur_word = kata_ganti(censor_char)
